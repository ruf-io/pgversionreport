...
export default function VersionMapCard({ data }: { data: Data }) {
    ...
    for (const [semver, date] of sortedVersions) {
      if (!major_versions[semver.major]) {
        ...
        if(semver.major === data.version.major && semver.minor > data.version.minor) {
    ...
    const current_version_release_date = getPgVersionDate(`${data.version.major}.${data.version.minor}`);
    const current_version_release_date_nice = current_version_release_date.toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
    const current_version_release_days_ago = Math.floor((new Date().getTime() - current_version_release_date.getTime()) / 86400000);

    const current_version_eol_date = new Date(eolDates[data.version.major]);
    const current_version_eol_date_nice = current_version_eol_date.toLocaleDateString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric",
      });
    const current_version_eol_days_remaining = Math.floor((current_version_eol_date.getTime() - new Date().getTime()) / 86400000);
  ...
  return (
    <Card className="p-2">
      <CardHeader className="p-4 pb-0">
        ...
        <CardDescription>
          You are <InlineCode>{minor_versions_behind}</InlineCode> minor versions behind and <InlineCode>{major_versions_behind}</InlineCode> major
          versions behind.
      ...
      <CardContent className="flex gap-4 p-4">
        <div className="flex-1 grid items-center gap-2">
          ...
          <div className="grid flex-1 auto-rows-min gap-0.5">
            <div className="text-sm text-muted-foreground">PG {data.version.major}.{data.version.minor} Release Date</div>
            <div className="flex items-baseline gap-1 text-xl font-bold tabular-nums leading-none">
                {current_version_release_date_nice} 
            </div>
            <div className="text-sm text-muted-foreground">
              <code>{current_version_release_days_ago}</code> days ago
            </div>
          </div>
          </div>
          <div className="flex-1 flex items-center gap-2">
          ...
          <div className="grid flex-1 auto-rows-min gap-0.5">
            <div className="flex gap-1 items-center text-sm text-muted-foreground">PG {data.version.major} EOL Date
            <Popover>
                  <PopoverTrigger>
                  ...
                  <PopoverContent>
                    <p className="text-sm">
                      Postgres major versions have approximately 5 years of support. After that, they are considered End of Life (EOL).
                    </p>
                  </PopoverContent>
                </Popover>
            </div>
            ...
            <div className="text-sm text-muted-foreground">
              in <code>{current_version_eol_days_remaining}</code> days
            </div>
          </div>
          </div>
        </div>
