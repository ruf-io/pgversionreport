...
type PerformanceImprovement = PromiseResolver<typeof data>["performanceImprovements"][0];
    ...
    cell: ({ row }) => (
      <CollapsibleDataItem title={row.getValue("title")}>
        <div className="prose prose-sm">
          <p>
            Fixed in: <InlineCode>{row.original.sinceVersion}</InlineCode>{" "}
            Contributor(s): {row.original.contributors.join(", ")}
          </p>
          <MarkdownBlock text={row.original.description} />
          <p><a href={generatePgDeepLink(
          row.original.sinceVersion,
          row.original.title
        )}
        target="_blank">View in PG Docs <ExternalLink className="inline h-4 w-4" /></a></p>
        </div>
      </CollapsibleDataItem>
    ),
  },
...
export function PerformanceDataTable({ data, version }: Props) {
  ...
  return (
    <div className="w-full">
      <div className="flex items-center py-4">
        <Input
          placeholder="Filter PerformanceImprovements..."
